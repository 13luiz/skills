...
*Blueprint generated by harness-audit.sh. For full scoring, run the agent-led audit (Mode 1 in SKILL.md).*
...
if [ "$PERSIST_MODE" = true ]; then
  ...
  if [ "$OUTPUT_FORMAT" = "markdown" ] || [ "$BLUEPRINT_MODE" = true ]; then
    EXT="md"
    SUFFIX=$([ "$BLUEPRINT_MODE" = true ] && echo "blueprint" || echo "audit")
  else
    EXT="json"
    SUFFIX="audit"
  fi
